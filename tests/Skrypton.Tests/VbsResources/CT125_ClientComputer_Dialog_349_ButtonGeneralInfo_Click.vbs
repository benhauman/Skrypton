...
Dim nexthinkBaseURL : nexthinkBaseURL = getNexthinkBaseURL() & "query?p1="
Dim nexthinkQuery: nexthinkQuery = "&platform=windows&query=(select (name last_ip_address group_name last_logged_on_user os_version_and_architecture device_manufacturer number_of_cpus cpu_model number_of_cores logical_cpu_number cpu_frequency total_ram number_of_graphical_cards graphical_card_ram last_system_boot last_logon_time bios_serial_number device_model ) (from device (where device (eq name (string %1))) ))&format=xml"
Dim nexthinkURL

Dim colorWarning : colorWarning = "#F20012"
Dim colorCheck :colorCheck  = "#1B709F"

Dim hostname: hostname = hlObj.GetValue("ComputerDetail.Hostname", 0, 0, 0, 0)
...
xmlhttp.setOption 2, 13056 'bypass certificate errors
