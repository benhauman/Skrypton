    ...
    internal sealed class OuterScopeBlockTranslator : CodeBlockTranslator
    {
        ...
        private void TranslateCode1Program(CSharpProgramCodeBuilder programBuilder, Tuple<string, int>[] dateLiteralsToValidateAtRuntime)
        {
            ...
            programBuilder.AddRange([new TranslatedStatement(TranslatedStatementKind.RawText, $"public sealed class {_startClassName.Name} : RunnerBaseT<{_envClassName.Name}, {_outerClassName.Name}>", 1, 0), // 'public sealed class Runner : '
                ...
                CSharpClassBuilder.CreateConstructor(2, 0, x => x.ClassName(_startClassName.Name)
                    .Parameter(typeof(IProvideVBScriptCompatFunctionalityToIndividualRequests).Name, "compatLayer", true)
                    .AddStatementRawText(_supportRefName.Name + " = compatLayer ?? throw new ArgumentNullException(nameof(compatLayer));", 0)
