...
namespace Skrypton.CSharpWriter.CodeTranslation.StatementTranslation
{
    ...
    public class FuncByRefArgumentMapper
    {
        ...
        /// call ends (even if it errors, since the argument may have been altered before the error). This function will identify which variables in an parsingExpression must be
        ...
        /// scopeAccessInformation reference does not indicate that the parsingExpression is within a function (or property) then there will be work to perform.
        /// </summary>
            ...
            NonNullImmutableList<FuncByRefMapping> rewrittenReferences)
        {
            ...
            return GetByRefArgumentsThatNeedRewriting(parsingExpression, scopeAccessInformation, rewrittenReferences, expressionIsDirectArgumentValue: false);
        }
            ...
            bool expressionIsDirectArgumentValue)
        {
            ...
            // any mappings should be read-only or read-write but it will miss an important case - if the "parsingExpression" at this point is a simple reference to a
            ...
            {
                var nameTokenTargetIfApplicable = TryToGetExpressionAsSingleNameToken(parsingExpression);
            ...
            return GetByRefArgumentsThatNeedRewriting(parsingExpression.Segments, byRefArguments, scopeAccessInformation, rewrittenReferences, expressionIsDirectArgumentValue);
        }
            ...
            bool expressionIsDirectArgumentValue)
        {
            ...
            {
                // If an codeExpression has multiple segments then no one part of it is elligible to be passed ByRef as a function argument - eg. "a + b" will not be
                // passed ByRef since that codeExpression "a + b" can not be altered by the receiving function even if the argument IS marked as ByRef.
            ...
            foreach (var expressionSegment in expressionSegments)
            {
                ...
                {
                    // If an codeExpression is wrapped in brackets then, even if it would otherwise be elligible to be passed as a direct / ByRef argument to a function,
                ...
                {
                    // This is a just a combined set of CallSetItemExpressionSegment instances, so we can handle this codeExpression segment type with more recursion
                ...
                throw new NotSupportedException("Unsupported codeExpression segment type: " + expressionSegment.GetType());
            }
            return rewrittenReferences;
        }
            ...
            int callSetItemCount)
        {
            ...
            // recorded that is not a read-only mapping; after the call codeExpression is evaluated, any changes to the alias reference must be mapped back onto the source function argument)
            ...
            // into a function as ByRef argument, that function can not affect the value of the codeExpression "a.Name", nor can it affect the value of "a". There sometimes-exception
            ...
            // Now, if error-trapping may be enabled, check for cases where a ByRef argument of the containing function/property is being referenced anywhere in the call codeExpression -
            ...
            // there will be no need to overwrite the original reference with the alias after the call codeExpression has been completed as the reference will not be passed to it ByRef
        ...
        private static NameToken? TryToGetExpressionAsSingleNameToken(ParsingExpression parsingExpression)
        {
            if (parsingExpression == null)
                throw new ArgumentNullException(nameof(parsingExpression));
            ...
            var callExpressionSegment = parsingExpression.Segments.Single() as CallExpressionSegment;
            if ((callExpressionSegment == null)
