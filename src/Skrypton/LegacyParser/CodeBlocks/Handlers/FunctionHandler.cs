...
namespace Skrypton.LegacyParser.CodeBlocks.Handlers
{
    internal sealed class FunctionHandler : AbstractBlockHandler
    {
        ...
        public override ICodeBlock Process(List<IToken> tokens)
        {
            ...
            bool isPublic = (!tokens[0].Content.Equals("PRIVATE", StringComparison.OrdinalIgnoreCase));
            ...
            || (blockType == BlockType.PrivateSub))
                return new SubBlock(isPublic, isDefault, new NameToken(false, funcNameToken.ContentUpperX(), funcNameToken.LineIndex), parameters, blockContent);

            else if ((blockType == BlockType.PublicFunction)
            || (blockType == BlockType.PublicDefaultFunction)
            || (blockType == BlockType.PrivateFunction))
                return new FunctionBlock(isPublic, isDefault, new NameToken(false, funcNameToken.ContentUpperX(), funcNameToken.LineIndex), parameters, blockContent);

            else if ((blockType == BlockType.PublicPropertyGet)
            || (blockType == BlockType.PublicDefaultPropertyGet)
            || (blockType == BlockType.PrivatePropertyGet))
                return new PropertyBlock(isPublic, isDefault, new NameToken(false, funcNameToken.ContentUpperX(), funcNameToken.LineIndex), PropertyBlock.PropertyType.Get, parameters, blockContent);

            else if ((blockType == BlockType.PublicPropertySet)
            || (blockType == BlockType.PrivatePropertySet))
                return new PropertyBlock(isPublic, isDefault, new NameToken(false, funcNameToken.ContentUpperX(), funcNameToken.LineIndex), PropertyBlock.PropertyType.Set, parameters, blockContent);
        ...
        private static List<FunctionBlock.Parameter> getFuncParametersAndRemoveTokens(
            List<IToken> tokens,
            int offset)
        {
        ...
        /// </summary>
        private static List<IToken> getParamTokens(List<IToken> tokens, int offset, out bool byRef, out string name, out bool isArray)
        {
            ...
            else if (token.Content.Equals("BYVAL", StringComparison.OrdinalIgnoreCase))
            {
                byRef = false;
                paramTokens.Add(token);
                offset++;
